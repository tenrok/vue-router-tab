...
export default {
  methods: {
     ...
     */
    async open(path, isReplace = false, refresh = true, isInheritable) {
      let { items } = this
      ...
      try {
        ...
        isInheritable && !isReplace
          ? (items.find(item => item.id === tarId).parentPath = curId)
          : NaN
      } catch (e) {
      } finally {
    ...
    async close() {
      ...
      try {
        const idx = items.findIndex(item => item.id === activeTabId)
        console.log(items)
        // 移除页签
        ...
        console.log(items.find(item => item.id === to))
        console.log(!items.find(item => item.id === to))
        // 如果关闭当前页签，则打开后一个页签
        if (!to || !items.find(item => item.id === to) || activeTabId !== id) {
